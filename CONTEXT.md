...
MV3 isolated world with the `chrome.*` API). The seven logic modules in
...
artifact is `dist/extension/` (a loadable MV3 folder, optionally zipped
as `dist/extension.zip` for the Web Store upload). Both artifacts share
the same `package.json` version and the same `src/` modules.
...
- **Chrome Web Store publish is manual for v0.3.0** — drag `dist/extension.zip` into the Developer Dashboard. CI auto-upload via the Chrome Web Store API is deferred until after the first publish cycle lands and we have real review feedback to design around.
