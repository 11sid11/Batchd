...
seven logic modules (`pacing.js`, `persist.js`, `failures.js`,
`selectors.js`, `run.js`, `panel.js`, `batchd.user.js`) change
semantically; only `batchd.user.js` loses the inline `gmStorage()`
factory and gains an import for `storage.gm.js`.

The Chrome Web Store publish step is manual for v0.3.0 (drag
`dist/extension.zip` into the Developer Dashboard) because the first
